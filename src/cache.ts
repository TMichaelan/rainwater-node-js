...
if (process.env.NODE_ENV !== 'test') {
    client.connect().catch(error => logger.error(`Database connection error: ${error}`));
}

export async function getCache(heights: string): Promise<number | null> {
    try {
        const res = await client.query('SELECT volume FROM cache WHERE heights = $1 AND created_at > NOW() - INTERVAL \'1 minute\'', [heights]);
...
export async function setCache(heights: string, volume: number): Promise<void> {
    try {
        await client.query('INSERT INTO cache (heights, volume, created_at) VALUES ($1, $2, NOW())', [heights, volume]);
